...
class InputObservable {
  ...
  subscribe(observer: Observer): void {
    this.obserwators.push(observer);
  }

  // ZAIMPLEMENTUJ: Metoda unsubscribe
  // Powinna usunąć obserwatora z listy
  unsubscribe(observer: Observer): void {
    this.obserwators = this.obserwators.filter((obs) => obs !== observer);
  }
  ...
  private notifyObservers(data: string): void {
    this.obserwators.forEach((obs) => obs.update(data));
  }
...
class ObserverComponent1 implements Observer {
  constructor(private element: HTMLElement) {
    this.element.textContent = 'oczekuje danych...';
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const inputElement = document.getElementById('observableInput') as HTMLInputElement;
  const observer1Element = document.querySelector(
    '[data-testid="observer-1-value"]'
  ) as HTMLElement;
  const observer2Element = document.querySelector(
